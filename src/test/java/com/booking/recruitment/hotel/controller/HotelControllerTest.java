...
class HotelControllerTest {
    ...
    @Test
    @DisplayName("When a specific hotel is requested by ID then it is returned")
    void hotelByIdRequested() throws Exception {
        Hotel hotel = repository.findById(1L).orElseThrow(
                () -> new IllegalStateException("Test dataset does not contain a hotel with ID 1!"));

        mockMvc
                .perform(get("/hotel/{hotelId}", 1L))
                .andExpect(status().is2xxSuccessful())
                .andExpect(jsonPath("$.id", equalTo(hotel.getId().intValue())))
                .andExpect(jsonPath("$.name", equalTo(hotel.getName())));
    }

    // TODO: Correct this test
    // @Test
    // @DisplayName("When a hotel deletion is requested then it is removed")
    // void hotelDeletedCorrectly() throws Exception {
    //     Long hotelId = 1L;
    //     mockMvc
    //             .perform(get("/hotel/{hotelId}", hotelId))
    //             .andExpect(status().is2xxSuccessful());

    //     mockMvc
    //             .perform(delete("/hotel/{hotelId}", hotelId))
    //             .andExpect(status().isOk());

    //     assertThat(repository.findById(hotelId).isPresent(), equalTo(false));
    // }
